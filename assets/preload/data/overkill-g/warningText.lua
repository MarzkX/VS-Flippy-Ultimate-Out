...
function onSongStart()
   setProperty('songLength', 138000)
end

function onCreate()
   makeLuaSprite('blackfuck', '', -200, -200)
   makeGraphic('blackfuck', 1920, 1080, '000000')
   setObjectCamera('blackfuck', 'camHUD')
   setProperty('blackfuck.alpha', 0)
   addLuaSprite('blackfuck', false)
end
...
function onStepHit()
 if mechanics then
  ...
  if curStep == 2464 then
   doTweenAlpha('lol', 'blackfuck', 1, 1, 'linear')
  end
  if curStep == 2560 then
     doTweenAlpha('lol', 'blackfuck', 0, 0.01, 'linear')
  end
   if curStep == 2528 then
      doTweenColor('TimeColor', 'timeBar', 'FF0000', 2, 'circInOut');
      runHaxeCode([[
