			...
			if(sender == weekFileInputText) {
				...
				for (i in 0...splittedText.length) {
					if(i >= weekFile.songs.length) { //Add new song
						weekFile.songs.push([splittedText[i], 'dad', [146, 113, 253], 'og']);
					} else { //Edit song
...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	private var menuSide:FlxSprite;
	...
	override function create() {
		...
		thisbg = new FlxSprite().loadGraphic(Paths.image('freeplay/fallen-soldier/og-bg'));
		thisbg.antialiasing = ClientPrefs.globalAntialiasing;
		thisbg.screenCenter();
		add(thisbg);

		thisfg = new FlxSprite().loadGraphic(Paths.image('freeplay/fallen-soldier/og-fg'));
		thisfg.antialiasing = ClientPrefs.globalAntialiasing;
		thisfg.screenCenter();
		add(thisfg);

		menuSide = new FlxSprite().loadGraphic(Paths.image('freeplay/menuSide'));
		menuSide.antialiasing = ClientPrefs.globalAntialiasing;
	...
	var thisbg:FlxSprite;
	var thisfg:FlxSprite;
	function freeplayChange()
	{
		if(bgArtText != null)
		{
			thisbg.loadGraphic(Paths.image('freeplay/fallen-soldier/' + bgArtText + '-bg'));
			thisfg.loadGraphic(Paths.image('freeplay/fallen-soldier/' + bgArtText + '-fg'));
		}
		else
		{
			thisbg.loadGraphic(Paths.image('freeplay/default'));
			thisfg.loadGraphic(Paths.image('freeplay/default'));
		}
	}
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			weekFile.songs[curSelected][3] = bgArtText.text;
	...
	function addFreeplayUI() {
		...
		bgArtText = new FlxUIInputText(120, bgColorStepperR.y + 70, 100, '', 8);
		...
		var artsDifferentCheckbox:FlxUICheckBox = new FlxUICheckBox(120, bgArtText.y + 30, null, null, "press T for another art?", 100);
		artsDifferentCheckbox.checked = weekFile.artsDifferent;
		artsDifferentCheckbox.callback = function()
		{
			weekFile.artsDifferent = artsDifferentCheckbox.checked;
		};
		...
		tab_group.add(new FlxText(120, bgArtText.y - 18, 0, 'Selected BG/Art:'));
		...
		tab_group.add(bgArtText);
		tab_group.add(hideFreeplayCheckbox);
		//tab_group.add(artsDifferentCheckbox);
		UI_box.addGroup(tab_group);
	}
	...
	function changeSelection(change:Int = 0) {
		...
		bgArtText.text = weekFile.songs[curSelected][3];
	...
	override function update(elapsed:Float) {
		...
		} else if(bgArtText.hasFocus) {
			FlxG.sound.muteKeys = [];
			FlxG.sound.volumeDownKeys = [];
			FlxG.sound.volumeUpKeys = [];
			if(FlxG.keys.justPressed.ENTER) {
				bgArtText.hasFocus = false;
			}
	    } else {
			FlxG.sound.muteKeys = TitleState.muteKeys;
			FlxG.sound.volumeDownKeys = TitleState.volumeDownKeys;
